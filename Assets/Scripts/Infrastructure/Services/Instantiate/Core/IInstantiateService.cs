...
namespace Infrastructure.Services.Instantiate.Core
{
    public interface IInstantiateService
    {
        ...
        public UniTask<T> InstantiateAsync<T>(T prefab, Transform parent, CancellationToken token = default) where T : Object;

        public UniTask<T> InstantiateAsync<T>(T prefab, Vector3 position, Quaternion rotation, CancellationToken token = default) where T : Object;

        public UniTask<T> InstantiateAsync<T>(T prefab, Vector3 position, Quaternion rotation, Transform parent, CancellationToken token = default) where T : Object;
    }
}